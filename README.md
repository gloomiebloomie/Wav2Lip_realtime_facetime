This is a fork from Wav2lip make a video using coquitts and whisper to simulate an ai facetime with text or speaking to it depending on hardware.First download the wav2lip_gan.pth and wav2lip.pth models from the wav2lip repo and place them in checkpoints folder. Do the same for the s3fd.pth to face_detection/detection/sfd/s3fd.pth  Next you need data record a 15-30 second wav of you voice in audacity or something then replace it with my iann.wav. For video make 512 by 512 square video of you talking for 5 seconds. I will add a script for resizing called resize video just put path of video in script then run it. Also reducing the framerate helps to make the video take way less time. When first loading coquitts yourtts model it may have an error where you need to search repo and find the issue, it is sovled replace code on line 100 of sample.py.you can do conda env for the python environment. The facetime text lets you text the ai from where you run python facetime script, use the grad.py on administrator on cmd first to start then run command in setup and instructions. It takes about 2-4 seconds for a response for me on a 8gb gpu. You can reduce the frames to even 1 -10 frames if you want in a one second video if your hardware smaller you can scale up if you have a better gpu. The other of pip installs matter to me but I did it in a conda env for setup. 
